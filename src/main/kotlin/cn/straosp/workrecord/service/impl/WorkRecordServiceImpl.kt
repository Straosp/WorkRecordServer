...
class WorkRecordServiceImpl : WorkRecordService {
    ...
    override fun addWorkRecord(accountId: Int, workRecord: AddWorkRecord): Result<Boolean> {
    ...
    override fun getLunarYearWorkSummary(accountId: Int, year: Int): YearWorkSummary {
        ...
        val salary = result.groupBy { it.teamSize }.map { (key, workRecords) ->
            key?.let {
                workRecords.sumOf { (it.singleProductPrice * it.singleProductQuantity) + (it.multipleProductPrice * it.multipleProductQuantity) }.div(key)
            }
        }.sumOf { it ?: .0 }
    ...
    override fun updateWorkRecord(accountId: Int, workRecordId: Int, workRecord: WorkRecord): Result<Boolean>{
        val wr = AppDatabase.database.from(WorkRecordTable).select()
            .where {
                (WorkRecordTable.id eq workRecordId) and (WorkRecordTable.accountId eq accountId) and (WorkRecordTable.workDate eq workRecord.workDate.toLocalDate())
            }.map { row ->
                row[WorkRecordTable.id] ?: 0
            }.toList() ?: emptyList()
        if (wr.isNotEmpty()){
            return Result.failure(RequestErrorMessage(Constant.UPDATE_WORK_RECORD_FAILED_CODE,Constant.UPDATE_WORK_RECORD_FAILED_MESSAGE))
        }
